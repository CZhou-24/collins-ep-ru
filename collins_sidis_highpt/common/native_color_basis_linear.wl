...
HSBasisCertificates[key_String,row_Association,known_Association,sectors_List,nominalKnown_:Automatic]:=Module[
 {nominal=If[nominalKnown===Automatic,known,nominalKnown],denominators,commonDen,products,
  ...
  colorRules,colorCoefficient,colorReconstruction},
 ...
 Do[index++;id=key<>"_b"<>IntegerString[index,10,3];scalar=piece["recipe"];originalScalar=scalar;normalizedTerms=<||>;localKnown=<||>;localNominal=<||>;refIndex=0;
  KeyValueMap[Function[{ref,factor},refIndex++;refID=id<>"_master"<>IntegerString[refIndex,10,3];
  ...
  (* Color and charge powers form an independent polynomial basis because
     the selected scalar integrals contain none of these external labels. *)
  colorVariables=Select[{SUNN,Nc,Nf,CF,CA,TF,eq,eqPrime},!FreeQ[scalar,#]&];
  gate["master numerators independent of color and charge labels",And@@(FreeQ[localNominal,#]&/@colorVariables)];
  If[colorVariables==={},colorRows={<|"monomial"->1,"recipe"->scalar|>},
   colorPieces=Join[<|"constant"->scalar["constant"]|>,scalar["terms"]];
   gate["cleared coefficients polynomial in color and charge",And@@(PolynomialQ[#,colorVariables]&/@Values[colorPieces])];
   ...
   gate["color polynomial reconstruction",Factor[scalar["constant"]-colorReconstruction["constant"]]===0&&
    And@@(Factor[scalar["terms"][#]-Lookup[colorReconstruction["terms"],#,0]]===0&/@Keys[scalar["terms"]])]];
  colorIndex=0;
 ...
 <|"values"->values,"basis"->basis,"recipes"->recipes,
  ...
  "reconstruction_residuals"->decomposition["individual_reconstruction_residuals"]|>];
