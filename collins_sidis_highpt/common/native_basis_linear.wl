...
HSBasisCertificates[key_String,row_Association,known_Association,sectors_List,nominalKnown_:Automatic]:=Module[
 {nominal=If[nominalKnown===Automatic,known,nominalKnown],denominators,commonDen,products,
  ...
  id,index=0,scalar,value,originalScalar,normalizedTerms,localKnown,localNominal,den,refID,refIndex},
 ...
 Do[index++;id=key<>"_b"<>IntegerString[index,10,3];scalar=piece["recipe"];originalScalar=scalar;normalizedTerms=<||>;localKnown=<||>;localNominal=<||>;refIndex=0;
  ...
  value=Expand[Total[Join[{scalar["constant"]},KeyValueMap[Factor[#2 localKnown[#1]]&,scalar["terms"]]]]];
  AssociateTo[values,id-><|"constant"->RUExact[scalar["constant"]],
   "terms"->KeyValueMap[<|"ref"->#1,"factor"->RUExact[#2]|>&,scalar["terms"]],
   "value"->RUExact[value],"eta_order"->0,"eps_order"->0,"sectors"->sectors|>];
  AssociateTo[basis,id->piece["basis"]/commonDen];AssociateTo[recipes,id->scalar];AssociateTo[scalarValues,id->value],{piece,decomposition["pieces"]}];
