...
LoadFC[] := (Global`$FAPatch=False;Global`$LoadAddOns={"FeynArts"};Global`$FeynCalcStartupMessages=False;
 Global`$FeynCalcDirectory=$StageContext["runtime"]["feyncalc_root"];
 FeynCalc`$FeynArtsDirectory=$StageContext["runtime"]["feynarts_root"];
 Get[FileNameJoin[{$StageContext["runtime"]["feyncalc_root"],"FeynCalc.m"}]];
